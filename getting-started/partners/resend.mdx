...
<Steps>
  ...
  <Step title="Add the connection in camelAI">
    ...
    <Note>
      Resend is a native connection, so the base URL and Bearer authentication are
      built in. There's nothing else to configure — just a name and the API key.
    </Note>
  </Step>
</Steps>

<Info>
  **Who can connect it.** Organization Owners and Admins create, update, and delete
  connections. Members can use a connection in their workspace, but can't manage it.
  Available on all plans. Resend bills your email volume, not camelAI.
</Info>

<Tip>
  While you're testing, send from Resend's shared `onboarding@resend.dev` address so you
  don't need a verified domain yet. Verify your own sending domain in Resend before you
  send production volume.
</Tip>
