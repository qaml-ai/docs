...
- We now support OpenAI's O3 model alongside the existing model
- Enter API keys for both models, and users will see a model switcher in the chat input field
- If you prefer to keep a single model configured, that's perfectly fine - the switcher only appears when multiple model API keys are provided
