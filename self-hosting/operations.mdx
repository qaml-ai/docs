...
For the default bundled-Pomerium direct-TLS deployment:
...
files. If `SELFHOST_POMERIUM_LOOPBACK_HTTPS=0`, omit only the loopback overlay.
...
Create a backup of durable application state, local Artifact repositories, and
...
organization. Also retain the TLS material and the release manifest used by
the installation.
...
Restore the matching `.env.selfhost`, TLS material, code release, and image
...
2. Verifies the target Git revision and all seven digest-pinned images.
...
- Protect `.env.selfhost`, Pomerium secrets, TLS keys, and backups.
...
Keep `.selfhost/pomerium/config.yaml`, secret files, and TLS keys owned by the
